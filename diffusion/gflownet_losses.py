...
def fwd_tb(initial_state, gfn, log_reward_fn, exploration_std=None, return_exp = False, threshold = -100):
    ...
    weight_positive = 20.0
    weight_negative = 1.0

    discrepancy = log_r + log_pbs.sum(-1) - log_pfs.sum(-1) - log_fs[:, 0]
    adv_reward = torch.where(discrepancy > 0, loss.detach() * weight_positive, loss.detach() * weight_negative)
    
    adv_reward = torch.log(torch.where(log_r < threshold, torch.tensor(1e-12, dtype=adv_reward.dtype, device=adv_reward.device), adv_reward) + 1.001)


    if return_exp:
        return loss.mean(), states, log_pfs, log_pbs, log_r, adv_reward.detach() 
    else:
        
        return loss.mean()


def bwd_tb(initial_state, gfn, log_reward_fn, exploration_std=None, logr = None, return_exp=False, is_adv = False, threshold = -100):
...
def fwd_tb_avg(initial_state, gfn, log_reward_fn, exploration_std=None, return_exp = False):
    ...
    if return_exp:
        return 0.5 * (loss ** 2).mean(), states, log_pfs, log_pbs, log_r, loss
    else:
...
def db(initial_state, gfn, log_reward_fn, exploration_std=None, return_exp = False):
    ...
    if return_exp:
        return loss.mean(), states, log_pfs, log_pbs, log_fs[:, -1], loss
    else:
