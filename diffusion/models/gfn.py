...
class GFN(nn.Module):
    ...
    def get_trajectory_fwd_off_policy(self, states, exploration_std, log_r, pis=False):
        bsz = states.shape[0]
        s = states[:, 0]
        logpf = torch.zeros((bsz, self.trajectory_length), device=self.device)
        logpb = torch.zeros((bsz, self.trajectory_length), device=self.device)
    ...
    def get_trajectory_back_and_forth(self, s, log_r, exploration_std, K, pis=False):
        bsz = s.shape[0]
        ...
        for i in range(self.trajectory_length-K, self.trajectory_length):
